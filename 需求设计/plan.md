...
**AI对话功能后端集成**：
- 小程序请求 → 后端API → AI服务提供商 → 后端处理 → 返回小程序
- 后端负责API密钥管理、成本控制、对话历史记录和缓存优化
...
**用户/会员表（users/members）**：
- 用户基本信息（ID、昵称、头像、联系方式等）
- 认证信息（登录凭证、密码等）
- 角色和权限
- 注册时间、最后登录时间等
...
**个人资料表（profiles）**：
- 与用户表关联
- 详细个人信息（真实姓名、性别、年龄、地址等）
- 技能标签、兴趣领域
- 个人简介、志愿服务经历

**积分记录表（points/credits）**：
- 与用户表关联
- 积分变动记录（获取/消耗原因、时间、数量）
- 积分类型（活动参与、任务完成、奖励等）
- 当前积分总量（可实时计算或缓存）
...
**活动参与记录表（activity_participants）**：
- 关联用户表和活动表
- 报名状态（已报名、已确认、已参加、已取消）
- 签到状态、完成状态
- 实际获得积分
...
**新闻通告表（news/announcements）**：
- 标题、摘要、正文内容
- 发布时间、生效期限
- 发布者信息
- 分类/标签（重要通知、活动预告、成果展示等）
- 状态（草稿、已发布、已下架）
- 阅读量统计
...
```javascript
   // 小程序中跳转到公众号文章
   wx.navigateTo({
     url: '/pages/webview/webview?url=https://mp.weixin.qq.com/s/xxxxx'
   });
```
